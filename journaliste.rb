...
puts journaliste.grep(/\d/).length
	# .grep vas chercher dans le array 
	# .length vas compter

    puts "Combien d'handle contiennent les 4 lettres du prénom \"Aude\" à la suite (sans faire attention à la casse) ?"

puts journaliste.grep(/aude/i).length
	# .grep vas chercher dans le array 
	# .length vas compter
	# le / /i ignoir les majuscule 

    puts "Combien commencent par une majuscule ?"

puts journaliste.grep(/@[A-Z]/).length

	# .grep vas chercher dans le array 
	# .length vas compter
	# le / /i ignoir les majuscule 
	# nous allons chercher apres le @


    puts "Combien contiennent une majuscule ?"

puts journaliste.grep(/[A-Z]/).size
	# .grep vas chercher dans le array des qui voie un occurence il passe au mot suivant  
	# .size vas compter
	# [A-Z] cherche les majuscule


   puts " Combien de _ dans tous les pseudos confondus ?"

i = 0    # on met i a 0
 journaliste.each {|x| i += x.count "_" } 
 	# .each va chercher dans l'array
 		# on incremente i et on calcul avec .count les "_" 
puts i
...
nouvelle_liste =  journaliste.sort_by! {|x|x.downcase}
	# .sort_by vas ranger dans l'ordre la liste 
	# le ! permet d'enregistrer dans le nouvelle array
	# le x.downcase vas ajouter les mots qui commence par une minuscule 
